...
**报告时间**: 2024-12-24
**分析范围**: 6个Vendor C科技项目与中国电信IoT平台的通信模式
**平台地址**: `https://device.api.ct10649.com:8743`

## 1. 执行摘要

本报告分析了Vendor C科技6个项目与中国电信物联网平台的通信规则，发现了统一的通信架构和几个关键差异点。所有项目都采用HTTPS双向认证，但在认证凭据、API版本和具体实现细节上存在差异。

### 1.1 核心发现
- **5个项目**使用生产环境 (`https://device.api.ct10649.com:8743`)
- **1个项目**使用开发环境 (`https://develop.api.ct10649.com:8743`) - 5G兼容需求项目
- **认证凭据分化**: 5个项目共享数据库配置凭据，1个项目(vendor-b)使用独立凭据
- API版本存在v1.1.0和v1.4.0两个主要版本
- 统一使用双向SSL证书认证
- **开发组织**: 公司A开发5个项目+主项目，公司B开发vendor-b项目

## 2. 项目概述

### 2.1 分析的6个项目
1. **信号覆盖主机内置式设备监控系统软件开发** (主项目)
2. **vendor-b项目** (zc_backend)
3. **Vendor C验收-物联网云监控平台**
4. **Vendor C验收-信号覆盖主机内置式设备监控系统**
5. **Vendor C验收-信号覆盖设备安装及租赁系统**
6. **Vendor C验收-5G兼容需求**
...
- vendor-b项目
- Vendor C验收-物联网云监控平台
- Vendor C验收-信号覆盖主机内置式设备监控系统
- Vendor C验收-信号覆盖设备安装及租赁系统
```
...
使用项目:
- Vendor C验收-5G兼容需求 (公司A开发)

配置文件: Vendor C验收/5G兼容需求/源码/source/src/main/resources/application.yml
配置内容:
  # iotplat:
...
### 4.2 vendor-b项目认证配置 (公司B开发) - 修正版
```
vendor-b项目APP_KEY: "ed5a4f1fcb364575a614f70d52a5a1ac"
vendor-b项目APP_SECRET: "f8a8df37f85a4b6892a7c058b5bfb655"

项目: vendor-b/zc_backend (公司B开发)
源码配置: vendor-b/zc_backend/nmps-impl/src/main/java/cn/com/git/nmps/impl/iot/utils/Constant.java
...
| vendor-b项目 | v1.1.0 | /iocm/app/dm/v1.1.0/devices         |
| Vendor C验收项目  | v1.1.0 | /iocm/app/dm/v1.1.0/devices         |
...
### 13.1 环境差异
- **5个项目**使用生产环境 (`https://device.api.ct10649.com:8743`)
  - 主项目 + 4个Vendor C验收项目 (公司A开发)
  - vendor-b项目 (公司B开发)
- **1个项目**使用开发环境 (`https://develop.api.ct10649.com:8743`)
  - Vendor C验收-5G兼容需求项目 (公司A开发)

### 13.2 认证凭据差异
- **5个项目**使用统一认证凭据 (来自数据库配置)
- **1个项目** (vendor-b) 使用独立认证凭据 (硬编码配置)

### 13.3 开发组织差异
- **公司A项目**: Vendor C验收相关的5个项目 + 主项目，技术栈和配置相对统一
- **公司B项目**: vendor-b项目，独立开发和配置
