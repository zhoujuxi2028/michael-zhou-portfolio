...
|:-------|:------|:--------|
|正式环境|10433748.api.ctwing.cn/aep_rule_engine|10433748.api.ctwing.cn/aep_rule_engine|
...
creator：创建人[*必填]
description:规则的描述信息, 长度不超过160位的字符串[*必填]
notifyType:消息类别；dataReport：数据上报（默认值），eventReport:事件上报，dataEventReport：数据事件上报
ruleStreams：规则信息[*必填][{   
     ruleStr:规则内容，即sql。不能包含特殊字符*，修改、新增时必填,
		具体详看下面SQL描述   
	 productId:产品ID[*必填]		
...
dataLevel:规则数据类型:0-产品级别;1-设备级别；2-设备分组级别[*必填]；组合规则只支持设备级别。
ruleType:规则类型；1000：简单规则，2000：组合规则
notifyType:消息类别；dataReport：数据上报，eventReport:事件上报，dataEventReport：数据事件上报
description:规则的描述信息, 长度不超过160位的字符串[*必填]
ruleStreams：规则信息[*必填][{   
     ruleStr:规则内容，即sql。不能包含特殊字符*，修改、新增时必填,
		具体详看下面SQL描述   
